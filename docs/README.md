...
高德地图环境创建`initMapSource(Opts: loaderOpts)`, 使用示例如下:
...
`createMapUtils(opts,loaderOPts)`为`MapUtils`的工厂函数, `opts`在`MapOptions`基础上扩展`mapIns,mountSelector`两个属性只能选择其中一个。
 `mapIns`表示地图已存在将其包装成`MapUtils`实例扩展相关功能, `mountSelector`表示地图不存在充当`MapUtils`工厂函数。
 `loaderOpts`加载器配置对象用于引入高德地图相关依赖 文档参照(loaderOpts)
[mapOptions配置对象](https://lbs.amap.com/api/javascript-api-v2/documentation#map)

...
# 基本功能
1. gdUtils
    1.1 前置: 地图配置保存，初始化操作, plugins
    2.1 地图事件绑定
    3.1 marker部分逻辑耦合在`gdMapUtils` 
2. markerPointerController 名称改为`markerLayer`
    2.1 点位管理:"添加、删除、更新、事件绑定 ( 图层事件管理 )"
    3.1 `markerList`经纬度变化同步更新位置
    4.1 marker激活状态管理
    5.1 图层管理："图层生命周期、图层销毁、图层事件绑定、图层marker自适应、marker查找"
3.  CollectionPointLayerController `markerClusterLayer`
    1. 图层管理：“显示,隐藏, 销毁, 图层生命周期”
4. LabelLayerRender 名称更改为`labelMarkerLayer`
> 通过抽象统一的接口，屏蔽地图服务差异，实现一次开发多方运行的目标。(为后续升级留下扩展接口)
## idea
```js
    ...
    // 地图示例已经创建，地图options无需传入，this.map保存着以上信息
```
...
4. 地图初始化从`gdMapUtils`中分离出来, 作为一个独立的函数`loadMapDep`，引入地图相关依赖

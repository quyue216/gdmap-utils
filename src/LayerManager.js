...
export class LayerManager {
   ...
   */
  static registerLayer(layerType, layerClass) {
    if (typeof layerType !== 'string' || typeof layerClass !== 'function') {
      console.error('[LayerManager Error]: Invalid layer type or layer class');
      return;
    }
    
    this.layerClassMap.set(layerType, layerClass);
    console.log(`[LayerManager]: Layer '${layerType}' registered with class '${layerClass.name}'`);
  }
   ...
   */
  static createLayer(layerType, options) {
    const LayerClass = this.layerClassMap.get(layerType);
    
    if (!LayerClass) {
      console.error(`[LayerManager Error]: Layer type '${layerType}' is not registered`);
      return null;
    }
   ...
   */
  static getLayerClass(layerType) {
    return this.layerClassMap.get(layerType);
  }
  ...
  static getAllLayerTypes() {
    return Array.from(this.layerClassMap.keys());
  }
   ...
   */
  static isLayerRegistered(layerType) {
    return this.layerClassMap.has(layerType);
  }
